const menuTemplate = ({ id, classes, height, width, zIndex, menu, scroll, page }) => `<div tabindex="-1" role="incontent_menu"
    ...
    switch (menuItem.type) {
        case "header":
            return `<li><span class="fw-content-menu-item-header" title="${menuItem.tooltip || ""}">${typeof menuItem.title === "function"
                ? menuItem.title(page)
                : menuItem.title}</span></li>`;
        case "separator":
            return '<li><hr class="fw-content-menu-item-divider"/></li>';
        default:
            return `<li tabindex="0" data-index="${index}" class="fw-content-menu-item content-menu-item${menuItem.disabled && menuItem.disabled(page)
                ? " fw-disabled"
                : menuItem.selected
                    ? " fw-selected"
...
export class ContentMenu {
    constructor({ id = false, page = false, classes = false, menu = { content: [] }, height = false, width = false, onClose = false, scroll = false, dialogEl = false, backdropEl = false, menuPos = false } = {}) {
        ...
        this.scroll = scroll;
        this.dialogEl = (dialogEl || null);
        this.backdropEl = (backdropEl || null);
    ...
    renderColumnsHtml(columns) {
        ...
        for (let col = 0; col < columns; col++) {
            ...
            for (let i = start; i < end; i++) {
                ...
                switch (menuItem.type) {
                    case "header":
                        html += `<li><span class="fw-content-menu-item-header" title="${menuItem.tooltip || ""}">${typeof menuItem.title === "function"
                    ...
                    case "separator":
                        ...
                        break;
                    default:
                        html += `<li tabindex="0" data-index="${i}" class="fw-content-menu-item content-menu-item${menuItem.disabled && menuItem.disabled(this.page)
                            ? " fw-disabled"
                            : menuItem.selected
                                ? " fw-selected"
                                : ""}" title='${menuItem.tooltip || ""}'>
    ...
    checkAndAddColumns() {
        ...
        if (dialogRect.height >= maxHeight) {
            const contentEl = this.dialogEl.querySelector(".fw-content-menu-content");
        ...
        return this.menu.content
            ...
            switch (menuItem.type) {
                ...
                case "separator":
                    return '<li><hr class="fw-content-menu-item-divider"/></li>';
                default:
                    return `<li tabindex="0" data-index="${index}" class="fw-content-menu-item content-menu-item${menuItem.disabled && menuItem.disabled(this.page)
                        ? " fw-disabled"
                        : menuItem.selected
                            ? " fw-selected"
                            : ""}" title='${menuItem.tooltip || ""}'>
    ...
    positionDialog() {
        ...
        let top = this.menuPos ? this.menuPos.Y : 0, left = this.menuPos ? this.menuPos.X : 0;
    ...
    getHighestDialogZIndex() {
        ...
        document
            ...
            .forEach(dialogEl => (zIndex = Math.max(zIndex, parseInt(dialogEl.style.zIndex) || 100)));
        document
    ...
    onclick(event) {
        ...
        const target = event.target.closest("li.fw-content-menu-item");
        if (target) {
            const menuNumber = target.dataset.index;
            if (menuNumber === undefined) {
                return;
            }
            const menuItem = this.menu.content[parseInt(menuNumber)];
