...
export class DialogTabs {
    ...
    render() {
        ...
        return `<div${wrapperAttrs}>
            <ul class="fw-tabs-nav">
                ${this.tabs
            .map((tab, index) => `<li class="fw-tab-link ${index === 0 ? "fw-current-tab" : ""}">
                                <a href="#${this.tabId(index, tab)}" class="fw-tab-link-inner" title="${escapeText(tab.description || "")}">
            ...
            ${this.tabs
            .map((tab, index) => `<div class="fw-tab-content fw-tabs-panel" id="${this.tabId(index, tab)}" title="${escapeText(tab.description || "")}">
                            ${tab.template()}
                        </div>`)
            .join("")}
        </div>`;
     ...
     */
    bind(container) {
        const tabLinks = Array.from(container.querySelectorAll(".fw-tabs-nav .fw-tab-link a"));
...
//# sourceMappingURL=dialog_tabs.js.map