...
export class PassphraseCrypto {
    ...
    static generateMasterKey() {
        return crypto.subtle.generateKey({ name: "AES-GCM", length: 256 }, true, // extractable for sessionStorage
        ["encrypt", "decrypt"]);
    }
    /**
    ...
    static generateKeyPair() {
        return crypto.subtle.generateKey({ name: "ECDH", namedCurve: "P-256" }, true, // extractable for encrypted storage
        ["deriveKey"]);
    }
    /**
    ...
    static async deriveKWK(passphrase, salt, iterations = 600000) {
        ...
        return crypto.subtle.deriveKey({
            ...
            iterations,
            hash: "SHA-256"
        }, keyMaterial, { name: "AES-GCM", length: 256 }, true, // extractable so it can be used to encrypt keys
        ["encrypt", "decrypt"]);
    }
    ...
    static unwrapKey(wrappedKeyBase64, wrappingKey, keyType = "AES-GCM") {
        ...
        const algorithm = keyType === "AES-GCM"
            ? { name: "AES-GCM", length: 256 }
            : { name: "HMAC", hash: "SHA-256", length: 256 };
        const usages = keyType === "AES-GCM" ? ["encrypt", "decrypt"] : ["sign", "verify"];
        return crypto.subtle.unwrapKey("raw", wrapped, wrappingKey, "AES-KW", algorithm, true, usages);
    }
    /**
    ...
    static unwrapPrivateKey(wrappedKeyBase64, wrappingKey) {
        const wrapped = PassphraseCrypto._base64ToBytes(wrappedKeyBase64);
        return crypto.subtle.unwrapKey("jwk", wrapped, wrappingKey, "AES-KW", { name: "ECDH", namedCurve: "P-256" }, true, ["deriveKey"]);
    }
    ...
    static async encryptKey(key, encryptionKey) {
        ...
        const ciphertext = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, encryptionKey, raw);
     ...
     */
    static async decryptKey(encryptedKeyBase64, encryptionKey, keyType = "AES-GCM") {
        const combined = PassphraseCrypto._base64ToBytes(encryptedKeyBase64);
        const iv = combined.slice(0, 12);
        const ciphertext = combined.slice(12);
        const raw = await crypto.subtle.decrypt({ name: "AES-GCM", iv }, encryptionKey, ciphertext);
        const algorithm = keyType === "AES-GCM"
            ? { name: "AES-GCM", length: 256 }
            : { name: "HMAC", hash: "SHA-256", length: 256 };
        const usages = keyType === "AES-GCM" ? ["encrypt", "decrypt"] : ["sign", "verify"];
    ...
    static async decryptPrivateKey(encryptedPrivateKeyBase64, encryptionKey) {
        const combined = PassphraseCrypto._base64ToBytes(encryptedPrivateKeyBase64);
        const iv = combined.slice(0, 12);
        const ciphertext = combined.slice(12);
    ...
    static async encryptDEKWithPublicKey(dek, recipientPublicKey) {
        ...
        const sharedKey = await crypto.subtle.deriveKey({ name: "ECDH", public: recipientPublicKey }, ephemeralPair.privateKey, { name: "AES-GCM", length: 256 }, false, ["encrypt", "decrypt"]);
        // Export DEK as raw bytes, then encrypt with shared key
        const dekRaw = await crypto.subtle.exportKey("raw", dek);
        const iv = crypto.getRandomValues(new Uint8Array(12));
        const ciphertext = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, sharedKey, dekRaw);
     ...
     */
    static async decryptDEKWithPrivateKey(encryptedDEKBase64, ephemeralPublicKeyJwk, privateKey) {
        // Import ephemeral public key
        const ephemeralPublicJwk = JSON.parse(ephemeralPublicKeyJwk);
        const ephemeralPublicKey = await crypto.subtle.importKey("jwk", ephemeralPublicJwk, { name: "ECDH", namedCurve: "P-256" }, true, []);
        // Derive shared AES-GCM key
        const sharedKey = await crypto.subtle.deriveKey({ name: "ECDH", public: ephemeralPublicKey }, privateKey, { name: "AES-GCM", length: 256 }, false, ["encrypt", "decrypt"]);
        // Decrypt DEK
        const combined = PassphraseCrypto._base64ToBytes(encryptedDEKBase64);
        const iv = combined.slice(0, 12);
        const ciphertext = combined.slice(12);
        const dekRaw = await crypto.subtle.decrypt({ name: "AES-GCM", iv }, sharedKey, ciphertext);
     ...
     */
    static async decryptStringWithPrivateKey(encryptedDataBase64, ephemeralPublicKeyJwk, privateKey) {
        const ephemeralPublicJwk = JSON.parse(ephemeralPublicKeyJwk);
        const ephemeralPublicKey = await crypto.subtle.importKey("jwk", ephemeralPublicJwk, { name: "ECDH", namedCurve: "P-256" }, true, []);
        const sharedKey = await crypto.subtle.deriveKey({ name: "ECDH", public: ephemeralPublicKey }, privateKey, { name: "AES-GCM", length: 256 }, false, ["encrypt", "decrypt"]);
    ...
    static async getKeysFromSession() {
        ...
        try {
            ...
            const masterKey = await crypto.subtle.importKey("raw", mkRaw, { name: "AES-GCM", length: 256 }, true, ["encrypt", "decrypt"]);
            const skJwk = JSON.parse(skJwkString);
            const privateKey = await crypto.subtle.importKey("jwk", skJwk, { name: "ECDH", namedCurve: "P-256" }, true, ["deriveKey"]);
    ...
    static hasKeysInSession() {
        return !!(sessionStorage.getItem("e2ee_master_key") &&
...
//# sourceMappingURL=passphrase-crypto.js.map