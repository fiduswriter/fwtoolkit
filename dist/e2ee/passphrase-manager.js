...
export class PassphraseManager {
    ...
    static async hasEncryptionKeys() {
        try {
            const data = (await getJson(apiUrl("e2ee.user_encryption_key")));
    ...
    static async setupEncryption(passphrase) {
        ...
        const encryptedMasterKey = await PassphraseCrypto.encryptKey(masterKey, kwk);
        const encryptedPrivateKey = await PassphraseCrypto.encryptPrivateKey(keyPair.privateKey, kwk);
        // 4. Encrypt backup of master key with recovery key
        const recoveryKeyRaw = PassphraseCrypto._hexToBytes(recoveryKey);
        const recoveryKeyCryptoKey = await crypto.subtle.importKey("raw", recoveryKeyRaw, { name: "AES-GCM", length: 256 }, false, ["encrypt", "decrypt"]);
        const encryptedMasterKeyBackup = await PassphraseCrypto.encryptKey(masterKey, recoveryKeyCryptoKey);
        // 5. Export public key as JWK
        const publicKeyJwk = await PassphraseCrypto.exportPublicKey(keyPair.publicKey);
        // 6. Send to server
        const saveData = {
        ...
        const { status } = await postJson(apiUrl("e2ee.user_encryption_key_save"), saveData);
        if (status >= 400) {
    ...
    static async unlockWithPassphrase(passphrase) {
        ...
        const kwk = await PassphraseCrypto.deriveKWK(passphrase, salt, data.user_iterations);
        // 3. Decrypt master key and private key
        const masterKey = await PassphraseCrypto.decryptKey(data.encrypted_master_key, kwk);
        const privateKey = await PassphraseCrypto.decryptPrivateKey(data.encrypted_private_key, kwk);
    ...
    static async changePassphrase(oldPassphrase, newPassphrase) {
        ...
        const oldKwk = await PassphraseCrypto.deriveKWK(oldPassphrase, oldSalt, data.user_iterations);
        const masterKey = await PassphraseCrypto.decryptKey(data.encrypted_master_key, oldKwk);
        const privateKey = await PassphraseCrypto.decryptPrivateKey(data.encrypted_private_key, oldKwk);
        ...
        const encryptedMasterKey = await PassphraseCrypto.encryptKey(masterKey, newKwk);
        const encryptedPrivateKey = await PassphraseCrypto.encryptPrivateKey(privateKey, newKwk);
    ...
    static async recoverWithRecoveryKey(recoveryKey, newPassphrase) {
        ...
        const recoveryKeyRaw = PassphraseCrypto._hexToBytes(recoveryKey);
        const recoveryKeyCryptoKey = await crypto.subtle.importKey("raw", recoveryKeyRaw, { name: "AES-GCM", length: 256 }, false, ["encrypt", "decrypt"]);
        const masterKey = await PassphraseCrypto.decryptKey(data.encrypted_master_key_backup, recoveryKeyCryptoKey);
        ...
        const encryptedMasterKey = await PassphraseCrypto.encryptKey(masterKey, newKwk);
        const encryptedPrivateKey = await PassphraseCrypto.encryptPrivateKey(newKeyPair.privateKey, newKwk);
        // 6. Create new backup
        const newRecoveryKeyRaw = PassphraseCrypto._hexToBytes(newRecoveryKey);
        const newRecoveryKeyCryptoKey = await crypto.subtle.importKey("raw", newRecoveryKeyRaw, { name: "AES-GCM", length: 256 }, false, ["encrypt", "decrypt"]);
        const encryptedMasterKeyBackup = await PassphraseCrypto.encryptKey(masterKey, newRecoveryKeyCryptoKey);
        // 7. Export new public key
        const publicKeyJwk = await PassphraseCrypto.exportPublicKey(newKeyPair.publicKey);
        ...
        await PassphraseCrypto.storeKeysInSession(masterKey, newKeyPair.privateKey);
    ...
    static async getDocumentPassword(documentId) {
        const { masterKey, privateKey } = await PassphraseCrypto.getKeysFromSession();
        if (!masterKey || !privateKey) {
            return null;
        }
        const { json } = await postJson(apiUrl("e2ee.document_encryption_key_get"), {
            document_id: documentId
        });
        const documentKeyData = json;
        if (!documentKeyData.has_key) {
            return null;
        }
        let password;
        if (documentKeyData.encrypted_with_master_key) {
            password = await PassphraseCrypto.decryptString(documentKeyData.encrypted_key, masterKey);
        }
        else {
            ...
            password = await PassphraseCrypto.decryptStringWithPrivateKey(parts[1], parts[0], privateKey);
            // Upgrade to master-key encryption for next time
            const upgradedEncryptedPassword = await PassphraseCrypto.encryptString(password, masterKey);
     ...
     */
    static async saveDocumentPassword(documentId, password, holderId = null, _holderType = "user", encryptedWithMasterKey = true) {
        let encryptedKey;
        if (encryptedWithMasterKey) {
            ...
            encryptedKey = await PassphraseCrypto.encryptString(password, masterKey);
        }
        else {
            if (holderId === null) {
                throw new Error("holderId is required for public-key encryption");
            }
            const pkJson = (await getJson(apiUrl("e2ee.user_public_key", { userId: String(holderId) })));
            if (!pkJson.has_key) {
                throw new Error("Recipient has not set up encryption");
            }
            const recipientPublicKey = await PassphraseCrypto.importPublicKey(pkJson.public_key);
            const encryptedData = await PassphraseCrypto.encryptStringWithPublicKey(password, recipientPublicKey);
        ...
        const { json, status } = await postJson(apiUrl("e2ee.document_encryption_key_save"), saveData);
        if (status >= 400) {
    ...
    static async userHasEncryptionKeys(userId) {
        try {
            const data = (await getJson(apiUrl("e2ee.user_public_key", { userId: String(userId) })));
            return data.has_key === true;
        }
        catch {
            return false;
    ...
    static async hasUserDismissedPassphraseOffer() {
        try {
            const data = (await getJson(apiUrl("user.preferences")));
...
//# sourceMappingURL=passphrase-manager.js.map