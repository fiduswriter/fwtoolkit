 ...
 */
export function enterPasswordDialog(onPassword, urlFragment = "", onCancel = null) {
        ...
        const buttons = [
            {
                ...
                click: () => {
                    ...
                    if (password.length === 0) {
                        const errorEl = document.getElementById("e2ee-password-error");
                        if (errorEl) {
                            errorEl.textContent = gettext("Please enter the password.");
                        }
                        return;
                    }
...
export function createPasswordDialog(onPassword) {
    return new Promise(resolve => {
        ...
        const buttons = [
            {
                ...
                classes: "fw-button fw-dark",
                click: () => {
                    const input = document.getElementById("e2ee-new-password-input");
                    const confirmInput = document.getElementById("e2ee-confirm-password-input");
                    const errorEl = document.getElementById("e2ee-password-error");
                    const password = input
                        ? input.value
                        : "";
                    ...
                    if (password !== confirmPassword) {
                        if (errorEl) {
                            errorEl.textContent = gettext("Passwords do not match.");
                        }
                        return;
                    }
                    dialogInstance.close();
        ...
        setTimeout(() => {
            ...
            const passwordInput = document.getElementById("e2ee-new-password-input");
            if (passwordInput) {
...
export function changePasswordDialog(onPasswordChange, options = {}) {
    return new Promise(resolve => {
        ...
        const showNewPasswordPlaintext = options.showNewPasswordPlaintext || false;
        ...
        const buttons = [
            {
                ...
                classes: "fw-button fw-dark",
                click: async () => {
                    const currentInput = document.getElementById("e2ee-current-password-input");
                    const newInput = document.getElementById("e2ee-new-password-input");
                    const confirmInput = document.getElementById("e2ee-confirm-password-input");
                    const errorEl = document.getElementById("e2ee-password-error");
                    ...
                    if (currentPasswordValue.length === 0) {
                        if (errorEl) {
                            errorEl.textContent = gettext("Please enter the current password.");
                        }
                        return;
                    }
                    ...
                    if (currentPasswordValue === newPassword) {
                        if (errorEl) {
                            errorEl.textContent = gettext("New password must be different from the current password.");
                        }
                        return;
                    }
                    dialogInstance.close();
            ...
            if (passwordInput && hasPrefilledNew) {
                passwordInput.addEventListener("input", () => {
                    if (passwordInput.value !==
                        ...
                        const confirmField = document.getElementById("e2ee-confirm-field");
                        if (confirmField) {
                            confirmField.style.display = "";
                        }
                        // Show strength meter again if user edits the prefilled value
                        const strengthMeter = document.querySelector(`#${dialogId} .e2ee-strength-meter`);
                        if (strengthMeter) {
                            ;
