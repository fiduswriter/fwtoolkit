...
export class FileDialog {
    ...
    updatePathDir(path) {
        const pathInput = this.dialog.dialogEl.querySelector("#path");
    ...
    init() {
                    ...
                    click: () => {
                        ...
                        if (this.movingFiles.length > 1) {
                            ...
                            this.movingFiles.forEach(doc => {
                                this.moveFile(doc, doc.path.endsWith("/")
                                    ? path
        ...
        this.fileSelector = new FileSelector({
            dom: this.dialog.dialogEl.querySelector(".fw-file-selector"),
            files: this.allFiles,
            showFiles: false,
    ...
    moveFile(file, requestedPath) {
        const promise = this.moveFunction
            ? moveFileWithFunction(file.id, file.title, requestedPath, this.moveFunction)
