...
export class ZipFileCreator {
    ...
    includeZips() {
        ...
        return Promise.all(getZipBlobs)
            ...
            return this.zipFiles.map(zipFile => {
                ...
                if (!zipDir) {
                    throw new Error(`Could not open zip folder "${zipFile.directory}".`);
                }
                if (zipFile.blob &&
                ...
                if (!zipFile.blob) {
                    throw new Error(`No blob available for zip file "${zipFile.directory}".`);
                }
    ...
    createZip() {
        ...
        const blobPromises = this.binaryFiles.map(binaryFile => {
            if (binaryFile.blob) {
                return blobToArrayBuffer(binaryFile.blob).then(ab => Promise.resolve({ data: ab, filename: binaryFile.filename }));
            }
