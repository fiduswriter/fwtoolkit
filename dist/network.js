...
export const get = (url, params = {}, csrfToken = false, signal) => {
...
export const getJson = (url, params = {}, csrfToken = false, signal) => get(url, params, csrfToken, signal).then(response => response.json());
...
export const postJson = (url, object = {}, files = {}, options = {}) => post(url, object, files, options).then(response => response.json().then(json => ({ json, status: response.status })));
