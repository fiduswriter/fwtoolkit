...
export class PulldownMenu {
        ...
        return `<div class="fw-pulldown-menu-bar" role="menubar">${this.options.menu.content
            .map(menu => `
                <div class="fw-pulldown-menu-header" data-id="${menu.id}">
                    <span class="fw-pulldown-menu-title${resolve(menu.disabled, this.context)
            ? " fw-disabled"
            : ""}"
                          title="${menu.tooltip
            ? escapeText(String(resolve(menu.tooltip, this.context)))
            : ""}"
            ...
            : ""}"
                          ...
                          aria-haspopup="true"
                          tabindex="0">
                        ${this.getAccessKeyHTML(resolve(menu.title, this.context), menu.keys?.slice(-1))}
                    </span>
    ...
    bind(container) {
        ...
        this.barEl = container.firstElementChild;
        this.bindEvents();
    }
    bindEvents() {
    ...
    onclick(event) {
        ...
        if (target.closest(".fw-pulldown-menu-item")) {
            const itemEl = target.closest(".fw-pulldown-menu-item");
        ...
        if (headerTitle &&
            ...
            const headerEl = headerTitle.closest(".fw-pulldown-menu-header");
            const menu = this.options.menu.content.find(item => item.id === headerEl.dataset.id);
            if (menu) {
                this.closeAllMenus();
                menu.open = true;
                this.openMenu = menu;
    ...
    findMenuItemByElement(element) {
        ...
        const headerEl = seekItem?.closest(".fw-pulldown-menu-header");
        if (!headerEl) {
            return null;
        }
        const menu = this.options.menu.content.find(item => item.id === headerEl.dataset.id);
    ...
    openSubMenu(menuItem) {
        ...
        const isChildOfParent = parent.content?.some(child => child.id === menuItem.id);
        if (isChildOfParent) {
            ...
            for (let index = this.parentChain.length - 2; index >= 0; index--) {
                ...
                if (ancestor.content?.some(child => child.id === menuItem.id)) {
                    ...
                    this.closeOtherMenus(ancestor, menuItem);
                    this.parentChain.push(menuItem);
                    foundAncestor = true;
                    break;
                }
    ...
    closeAllMenus(menu = this.options.menu) {
        menu.content.forEach(item => {
    ...
    closeOtherMenus(menu, currentMenuItem) {
        menu.content.forEach(item => {
            if (item.type === "menu" && item.open) {
                if (!this.parentChain.includes(item) &&
                    item.id !== currentMenuItem.id) {
                    item.open = false;
                }
                if (item.content) {
                    this.closeOtherMenus(item, currentMenuItem);
                }
            }
        });
    }
    ...
    changeCursorMenuItem(name) {
        ...
        if (!this.cursorMenuItem) {
            ...
            if (this.parentChain.length > 1) {
                ...
                this.closeAllMenus(this.parentChain[this.parentChain.length - 1]);
            }
            else {
                const currentMenuIndex = this.options.menu.content.findIndex(menu => menu.open);
            ...
            if (this.cursorMenuItem.type === "menu") {
                ...
                const currentMenuIndex = this.options.menu.content.findIndex(menu => menu.open);
                const newMenuIndex = currentMenuIndex === this.options.menu.content.length - 1
                    ? 0
                    : currentMenuIndex + 1;
                if (this.openMenu) {
                    this.openMenu.open = false;
    ...
    getActionMenuItemHTML(menuItem) {
        ...
        const tooltip = menuItem.tooltip
            ...
            : undefined;
        return `<span class="fw-pulldown-menu-item fw-pulldown-item${selected ? " fw-selected" : ""}${disabled ? " fw-disabled" : ""}${menuItem === this.cursorMenuItem ? " fw-cursor" : ""}"
        role="menuitem"
        tabindex="0"
        data-id="${menuItem.id}"
...
//# sourceMappingURL=pulldown_menu.js.map