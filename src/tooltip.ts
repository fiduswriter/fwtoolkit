...
const getTrigger = (target: EventTarget | null): HTMLElement | null => {
    ...
    return target.closest(TRIGGER_SELECTOR) as HTMLElement | null
}

const getTooltipText = (trigger: HTMLElement): HTMLElement | null => {
    const selector = trigger.matches(INFO_TOOLTIP_SELECTOR)
        ? ".fw-info-tooltip-text"
        : ".fw-tooltip"
    return trigger.querySelector(selector) as HTMLElement | null
}

const isTopPlacement = (trigger: HTMLElement): boolean =>
    trigger.matches(INFO_TOOLTIP_SELECTOR)

const getZIndex = (): number => {
    let zIndex = 1000
    document.querySelectorAll(".fw-dialog").forEach(dialogEl => {
        const dialogZ = parseInt(
...
const positionTooltip = (tooltip: HTMLElement, trigger: HTMLElement): void => {
...
const hideTooltip = (): void => {
    ...
    if (trigger.isConnected && tooltipOrigin) {
        ...
        if (tooltipOrigin.nextSibling) {
            tooltipOrigin.parent.insertBefore(
                tooltip,
                tooltipOrigin.nextSibling
            )
        } else {
...
const shouldStayVisible = (): boolean => {
    if (!activeTrigger) {
        return false
    }
